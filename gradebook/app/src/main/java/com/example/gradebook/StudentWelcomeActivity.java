...
public class StudentWelcomeActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.i("gradebook", "Student viewing courses");

        ListView coursesStudentView = (ListView) findViewById(R.id.course_progress_list);
        ...
        coursesStudentView.setAdapter(arrayAdapter);

        //add listener to coursesStudentView
        final Intent intent = new Intent(this, GetProgressActivity.class);
        coursesStudentView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                intent.putExtra("token", token);
                intent.putExtra("courseName", courseArrayList.get(i));
                startActivity(intent);
            }
        });
    }

    public void addCourseStudent(View view) {
    ...
    public void studentLogOut(View view) {
        Log.i("Gradebook", "Student logging out");
        ServerCommunicator message = new ServerCommunicator();
        String response = "";
        try {
            response = message.execute(GradebookProtocol.TYPE_KEY + GradebookProtocol.PARAM_ASSIGN +
